...
export function QuoteForm({ title = 'Cotizador rápido' }: { title?: string }) {
  return (
    <div className="bg-[#fafaf8] border border-zinc-100 p-8 lg:p-10">
      <h3 className="font-['Playfair_Display'] text-xl font-semibold text-zinc-900 mb-8">{title}</h3>
      <form className="space-y-4" onSubmit={e => e.preventDefault()}>
        ...
        <select className={`${inputClass} appearance-none text-zinc-500`}>
          <option>Tipo de servicio</option>
          <option>Despacho Aduanal</option>
        ...
        <label className="flex items-start gap-3 cursor-pointer">
          <input type="checkbox" className="mt-1 accent-[#c41e3a]" />
          <span className="text-xs text-zinc-500 leading-relaxed">
            Acepto el <a href="/aviso-de-privacidad" className="text-[#c41e3a] underline">Aviso de Privacidad</a>
          </span>
        </label>
        <button type="submit"
